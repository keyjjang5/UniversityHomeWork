	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char*)"../DirectX_tutorial/Data/fontdata.txt", (WCHAR*)L"../DirectX_tutorial/Data/font.dds");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, (char*)"Barrel", 400, 500, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, (char*)"Deer", 525, 500, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, (char*)"Crate", 650, 500, 1.0f, 0.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, (char*)"Wolf", 775, 500, 0.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, (char*)"Cotage", 900, 500, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
