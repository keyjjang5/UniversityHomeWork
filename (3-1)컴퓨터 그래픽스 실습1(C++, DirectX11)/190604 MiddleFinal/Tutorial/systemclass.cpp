...
bool SystemClass::Frame()
{
	...
	//if (m_Input->IsKeyPressed(DIK_W)) m_Graphics->MoveCamera(0.0f, 0.0f, 0.08f);
	//else if(m_Input->IsKeyPressed(DIK_S)) m_Graphics->MoveCamera(0.0f, 0.0f, -0.08f);
	//else if (m_Input->IsKeyPressed(DIK_A)) m_Graphics->MoveCamera(-0.08f, 0.0f, 0.0f);
	//else if (m_Input->IsKeyPressed(DIK_D)) m_Graphics->MoveCamera(0.08f, 0.0f, 0.0f);
	...
	result = m_Graphics->Frame(m_Fps->GetFps(), m_Cpu->GetCpuPercentage(), m_Obj->GetCntObject(), m_Poly->GetCntPolygon(),
		m_ScreenSize->GetScreenSizeX(), m_ScreenSize->GetScreenSizeY(), mouseX, mouseY, m_Timer->GetTime());
	if (!result)
	{
		return false;
	}
