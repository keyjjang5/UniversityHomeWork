...
TextClass::TextClass()
{
	...
	for (int i = 0; i < 9; i++) m_sentence[i] = 0;
}
...
void TextClass::Shutdown()
{
	...
	for (int i = 0; i < 9; i++) ReleaseSentence(&m_sentence[i]);
...
bool TextClass::SetCameraPos(D3DXVECTOR3 pos, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence[6], CameraPosString, 20, 140, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result) return false;
...
bool TextClass::SetCameraRot(D3DXVECTOR3 rot, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence[7], CameraRotString, 20, 160, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result) return false;
