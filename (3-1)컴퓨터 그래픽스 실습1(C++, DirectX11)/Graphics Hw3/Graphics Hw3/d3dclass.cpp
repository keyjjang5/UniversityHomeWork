	...
	float screenDepth, float screenNear)
{
	...
	// wcstombs_s에서 인수호출을 못하는 문제가 지속적으로 발생, 검색결과 unsigned int 형인 stringLength를 형태에 맞는 size_t로 바꾸라는 의견이 있었음
	// 그리고 현재(튜토리얼3)기준으로 Initialize에서도 선언과 wcstombs_s에서 밖에 stringLength를 사용하지 않아서 변수형을 바꿈
	size_t stringLength;
...
void D3DClass::WriteVideoCardInfo()
{
	using namespace std;
	// 파일 쓰기
	// 매번 파일을 새로 생성
	ofstream outFile;
	outFile.open("videoCardInfo.txt", ios::trunc);

	char cardName[128];
	int memory;
	GetVideoCardInfo(cardName, memory);
	outFile << cardName << endl;
	outFile << memory << endl;
	...
	// 공부 겸 써놓음, 실제 사용은 안함
	ifstream inputFile;
	inputFile.open("videoCardInfo.txt", ios::in);
